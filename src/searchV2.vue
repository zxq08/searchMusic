...
export default {
  ...
  mounted () {
    ...
    this.getStorage('historyList', res => {
      if (res) {
  ...
  methods: {
    inputSearch () {
      if (this.searchName) {
        axios.get(`/search/suggest?keywords=${this.searchName}&type=mobile`).then(res => {
          var data = res.data.result
          this.searchList = data.allMatch
          this.searchType = 3
        })
      } else {
        this.searchType = 1
    ...
    search () {
      ...
      this.setStorage('historyList', this.historyList)
      axios.get(`/search?keywords=${this.searchName}`).then(res => {
        var data = res.data.result
        this.resultList = data.songs
        this.searchType = 2
      })
    },
    searchHis (hiskey) {
      this.searchName = hiskey
      this.search()
    },
    play ({ id }) {
      console.log('play:' + id)
